---
title: Workflows
description: YAML templates that materialize as issues, dependencies, and labels — with optional human approval gates.
---

![A workflow pipeline with a glowing approval gate in the middle](/img/workflow-gate.png)
...
JavaScript](/docs/dynamic-workflows) and pipe it into `clu batch` instead.
</Callout>
...
`vars` declares typed variables that must be supplied at run time. `needs` is
...
```bash
clu run release -v version=1.2.3
```

That single command creates the parent issue, four children, and all dep edges
...
- a `checkpoint:pending` label, and
- a `cp:<issue-id>` KV entry holding `{kind, approvers}` JSON.
...
See [`demo-workflow.sh`](https://github.com/arjia-labs/clu/blob/main/demo-workflow.sh)
in the repo for the full demo.

## Architecture

Workflows are split in two:

- `internal/workflow/` is pure — YAML loader, validation, variable resolution,
  `MakePlan(Template, vars) → Plan`. No store calls.
- `internal/cli/run.go` walks the Plan, calls `store.Create` per step, writes
  deps and the run/step labels.

This keeps the planner trivially testable and the storage layer free of
template concepts.
