---
title: Importing & bulk graphs
description: Turn one JSON document into a whole validated issue graph with clu batch — and pipe issues in from a Linear or Notion CLI.
---

![Many data streams converging through a funnel into one dependency graph](/img/import-funnel.png)
...
The producer can be anything that emits JSON — a shell script, a Node
...
- **Generation** — *any* language, *any* logic: loops, conditionals, computed
  fan-out, reading an API to decide the shape. `clu` doesn't care what
  produced the graph.
- **Instantiation** (`clu batch`) — validation, ID allocation, and one atomic
  write. "Graphed correctly" is *guaranteed* here, not hoped for in the
  generator.

This is the programmable alternative to a static [`clu run`](/docs/workflows)
...
| `capabilities` | [string] | Stored as `cap:<name>` routing labels (see [capability routing](/docs/multi-agent)). |
...
A `group` field in the document does the same; the `--group` flag wins if both
are present.
...
- **`skip`** (default) — the existing issue is left untouched; only
  genuinely-new issues are added.
- **`update`** — the issue's *source-owned* fields (`title`, `type`,
  `priority`, `description`) are re-synced; local workflow state (status,
  assignee, labels, deps, notes) is left alone.
...
The contract is just JSON, so any tracker with a CLI or API can feed `clu
batch`. The recipe is always the same: **dump → map to the batch shape (give
each issue a stable `key`) → pipe**.

### Linear

A ready-made generator lives at
[`examples/generators/linear-todo.js`](https://github.com/arjia-labs/clu/blob/main/examples/generators/linear-todo.js).
...
Adjust the property paths to your Notion schema and whichever `notion`/`ntn`
...
[`export`/`import`](#backup--restore) round-trip below.

## The `clu.js` helper (optional)

For Node generators, [`examples/generators/clu.js`](https://github.com/arjia-labs/clu/blob/main/examples/generators/clu.js)
...
`Graph.phase(name, fn)` groups issues into ordered stages: a phase can't start
...
generators in [`examples/generators/`](https://github.com/arjia-labs/clu/tree/main/examples/generators):
...
`import` is the inverse of `export`; `--lenient` skips unparseable lines. Reach
